...
The data has three different data types
Numeric: record id, month, day, year, plot id, hindfoot length, and weight.
Character: species id, sex, day of week, plot type, genus, species, and taxa.
Date: date
...
ggplot(data = surveys, mapping = aes(x = weight, y = species)) +  
       geom_boxplot(outlier.shape = NA) +
  geom_jitter(color = "cadetblue")+
  labs(x = "Weight (grams)", y = "Species of Rodent", title = "Species of Rodent v Weight")
```
...
# Resubmtion Reflection
The first change I made was using str() for question 3 to find the type of data in each variables in the data set. This function printed out each row of the data set along with the data type. My thinking changed because I realized that there is a different data type in each of rows of data and I needed a function that could give me all of this information. The second change I made was in question 5. I moved alpha from the aesthetics to inside geom_point. My thinking changed here because I realized that alpha is not being mapped to a variable in the data set and thus, does not need to be in the aesthetics. 
